...
{
    public partial class frmPesquisaLeitor : Form
    {
        public frmPesquisaLeitor()
        {
            InitializeComponent();
        }
        ...
        private void txtLeitor_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string sql = @"SELECT * FROM LEITORES
                    WHERE NOME LIKE '" + txtLeitor.Text + "%'";
        ...
        private void frmPesquisaLeitor_Load(object sender, EventArgs e)
        {
            try
            {
                Conexao.Conectar();
